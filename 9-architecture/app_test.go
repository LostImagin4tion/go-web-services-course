...
func TestApp(t *testing.T) {
	...
	testCases := []*ApiTestCase{
		...
		},
		{
			...
			After: func(r *http.Response, body []byte, resp interface{}) error {
				...
				templateParams["token1"] = val.String()
				return nil
			},
		},
		{
		...
		},
		{
			...
			Before: func() {
				templateParams["EMAIL"] = "u_" + templateParams["EMAIL"]
			},
			After: func(r *http.Response, body []byte, resp interface{}) error {
				val, err := lookup.LookupString(resp, "User.Token")
				if err != nil {
		...
		},
		{
			...
			After: func(r *http.Response, body []byte, resp interface{}) error {
				...
				templateParams["token2"] = val.String()
				return nil
			},
		},

		{
			...
			After: func(r *http.Response, body []byte, resp interface{}) error {
				...
				templateParams["slug1"] = val.String()
				return nil
			},
		},
		{
			...
			After: func(r *http.Response, body []byte, resp interface{}) error {
				...
				templateParams["slug2"] = val.String()
				return nil
			},
		},
		{
	...
	for _, item := range testCases {
		ok := t.Run(item.Name, func(t *testing.T) {
			...
			if item.TokenName != "" {
				req.Header.Add("Authorization", "Token "+templateParams[item.TokenName])
			}
