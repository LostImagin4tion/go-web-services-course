...
* SingleHash:
  * считает значение `crc32(data)+"~"+crc32(md5(data))`, где `data` - то что пришло на вход
* MultiHash:
  * считает значение `crc32(th+data))` (конкатенация цифры, приведённой к строке и строки), где `th=0..5` ( т.е. 6 хешей на каждое входящее значение )
  * потом берёт конкатенацию результатов в порядке расчета, где `data` - то что пришло на вход (и ушло на выход из `SingleHash`)
* CombineResults:
